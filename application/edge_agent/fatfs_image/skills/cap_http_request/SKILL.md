...
## Läsa Sigges egna sensorer (Home Assistant REST API)

Sigges sensordata finns i HA. Läs dem så här:

```json
{
  "url": "http://192.168.86.73:8123/api/states/sensor.sigge_avstand",
  "headers": { "Authorization": "Bearer <HA_LONG_LIVED_TOKEN>" }
}
```

Svaret innehåller `"state": "432"` (avstånd i mm). Använd `sensor.sigge_batteri` för batteri.
Token skapas i HA under Profil → Säkerhet → Långlivade åtkomsttokens.
